...
**Definition.** Memory consolidation is a transformation function *C: H → M*, where *H* is an agent's interaction history and *M* is a persistent compressed representation that preserves relational and causal continuity across context resets.
...
**Prediction:** Agents using Dream Weaver consolidation will demonstrate higher memory tool reuse rates and lower task abandonment rates compared to agents relying solely on retrieval-based memory (layers 1-3), when measured across sessions spanning more than 7 days. Specifically, we predict that the memory search invocation rate (Section 11.1) will remain above 50% of sessions after consolidation, versus the sub-30% rate observed during the fading period in our deployment.
...
**Complexity.** Time: *O(n)* where *n* is total transcript length. Space: *O(m)* where *m* is consolidated narrative size, *m << n*. In our deployment, 119MB of transcripts consolidated to ~8K words per agent.
